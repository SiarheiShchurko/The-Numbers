...
class SettingTableVC: UITableViewController {
    ...
    func loadSettings() {   ///Эта функция для загрузки настроек. Чтобы они отражались на дисплее.
        ...
        sWitch.isOn = SettingsClass.shared.currentSettings.timerOn ///Указывается что положение свитча будет указываться исходя из того, false or true положение timerOn.
    }
