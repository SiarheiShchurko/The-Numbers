...
//        SetDispBase.shared.currentSettings.timerOn = sender.isOn ///Свитч включен если таймер тру
//    }
//    func loadSettings() {   ///Эта функция для загрузки настроек. Чтобы они отражались на дисплее.
//        secondsChangeOut.text = "\(SetDispBase.shared.currentSettings.timeForGame) sec" ///Для настройки времени используем оутлет для лэйбла 30сек. Указывается - что будет отражаться время выбранное в timeForGame
//        sWitch.isOn = SetDispBase.shared.currentSettings.timerOn ///Указывается что положение свитча будет указываться исходя из того, false or true положение timerOn.
//    }
//    @IBAction func DefaultSetAction(_ sender: Any) {
//        SetDispBase.shared.() ///Обнуляем настройки по умолчанию.
